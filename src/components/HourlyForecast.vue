...
<template>
    ...
    <div class="px-2">
      ...
      <div class="flex overflow-x-hidden pb-2">
        <div
          v-for="n in 8"
          :key="n"
          class="flex h-24 w-[4.5rem] shrink-0 flex-col items-center justify-center gap-2 border-r border-slate-200 last:border-r-0 dark:border-slate-800"
        >
  ...
  <WidgetError
    ...
    :title="languageStore.t('hourly.error')"
    :error="error"
  />
  ...
  <!-- Forecast card                                                           -->
  <!-- -------------------------------------------------------------------- -->
  <WidgetContainer v-else-if="forecast">
    <template #loading-bar>
      <WidgetLoadingBar v-if="loading" />
    </template>

    <WidgetHeader :title="languageStore.t('hourly.title')" />
    ...
    <div class="relative">
      <div class="-mx-1 flex snap-x snap-mandatory gap-2 overflow-x-auto px-1 pb-2 pr-8">
        ...
        >
          <!-- Time -->
          <span
            class="text-[11px] uppercase tracking-[0.18em] text-storm-water-500 dark:text-sea-mist-300/65"
            >{{ card.time }}</span
          >
          <!-- Weather icon -->
          <WeatherIcon
          ...
          <!-- Wind -->
          <div
            v-if="card.windSpeed !== null"
            class="flex items-baseline gap-1 text-storm-water-500 dark:text-sea-mist-300/70"
          >
            <span class="text-[10px] font-medium">{{ Math.round(card.windSpeed) }}</span>
            <span class="text-[9px] uppercase tracking-wider">{{
              card.windDirection !== null ? degreesToCompass(card.windDirection) : ''
            }}</span>
          </div>
          ...
          <span class="text-[10px] text-storm-water-400 dark:text-[#d7b07c]/70"
            >{{ card.precipProb }}%</span
          >
        </div>
      </div>
      ...
      <div
        class="pointer-events-none absolute -right-2 bottom-0 top-0 flex w-10 items-center justify-end bg-gradient-to-l from-white via-white/80 to-transparent pb-2 pr-1 dark:from-slate-950 dark:via-slate-950/80"
      >
        <svg
          class="size-4 animate-pulse text-storm-water-400 dark:text-sea-mist-300/50"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
