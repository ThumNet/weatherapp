...
function formatTime(iso: string): string {
  ...
  return d.toLocaleTimeString(languageStore.locale, {
    hour: '2-digit',
    minute: '2-digit',
    hour12: false,
  })
...
const todayOutlook = computed(() => {
  ...
  if (precipProb >= 70 && precipHours >= 4)
    rainPhrase = languageStore.t('current.rainLikelyMuchOfDay')
...
<template>
  <div>
      ...
      <div class="py-6">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div
            v-for="i in 4"
            :key="i"
            class="h-16 animate-pulse rounded-xl bg-slate-200 dark:bg-white/20"
          />
        </div>
    ...
    <WidgetError
      ...
      :title="languageStore.t('current.error')"
      :error="error"
    />
    ...
    <!-- Weather card                                                          -->
    <!-- ------------------------------------------------------------------ -->
    <WidgetContainer v-else-if="weather" class="border-b-0 !px-0 pb-0 sm:!px-0">
      <template #loading-bar>
        <WidgetLoadingBar v-if="loading" />
      </template>
      ...
      <div class="py-6">
        ...
        <div class="mb-8 flex flex-col items-center justify-center gap-4 px-4 sm:px-6">
          <div class="flex items-center gap-4">
            <div class="flex items-start gap-1 leading-none">
              <span
                data-display="true"
                class="font-display text-[4.5rem] font-medium tracking-[-0.05em] text-dutch-orange sm:text-[4.75rem]"
              >
                {{ temperature }}
              </span>
              <span
                class="mt-2.5 text-[1.7rem] font-normal text-storm-water-500 dark:text-sea-mist-200/80"
                >°C</span
              >
            </div>
            <WeatherIcon
          ...
          <div class="min-w-0">
            <p
              class="max-w-md text-center text-[10px] uppercase leading-relaxed tracking-[0.15em] text-storm-water-500 dark:text-sea-mist-300/70"
            >
              {{ todayOutlook }}
            </p>
          </div>
        ...
        <div class="grid grid-cols-2 border-y border-slate-200 dark:border-slate-800">
          ...
          >
            <svg
              class="size-5 text-storm-water-500 dark:text-sea-mist-300/70"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="1.75"
            ...
            <span
              class="text-[10px] font-medium uppercase tracking-[0.12em] text-storm-water-500 dark:text-sea-mist-300/65"
              >{{ languageStore.t('current.feelsLike') }}</span
            >
          ...
          >
            ...
            <span
              class="text-[10px] font-medium uppercase tracking-[0.12em] text-storm-water-500 dark:text-sea-mist-300/65"
              >{{ languageStore.t('current.humidity') }}</span
            >
            <span class="text-[15px] font-semibold text-storm-water-800 dark:text-dune-foam"
          ...
          >
            ...
            <span
              class="text-[10px] font-medium uppercase tracking-[0.12em] text-storm-water-500 dark:text-sea-mist-300/65"
              >{{ languageStore.t('current.wind') }}</span
            >
            <span class="text-[15px] font-semibold text-storm-water-800 dark:text-dune-foam">
              {{ Math.round(weather.windSpeed) }}
              <span class="text-xs font-normal text-storm-water-500 dark:text-sea-mist-300/70"
                >km/h</span
              >
              {{ windCompass }}
            </span>
          ...
          >
            <svg
              ...
              aria-hidden="true"
              v-html="moonPhase.phaseIcon"
            />
            <span
              class="text-[10px] font-medium uppercase tracking-[0.12em] text-storm-water-500 dark:text-sea-mist-300/65"
              >{{ languageStore.t('current.moon') }}</span
            >
            <span
              class="text-[15px] font-semibold leading-tight text-storm-water-800 dark:text-dune-foam"
              >{{ languageStore.t(`moon.${moonPhase.phaseKey}`) }}</span
            >
          </div>
        </div>
        ...
        >
          <!-- Sunrise -->
          <div
            class="flex flex-col items-center justify-center gap-1 border-r border-slate-200 px-2 py-5 text-center dark:border-slate-800"
          >
            ...
            <span
              class="text-[10px] font-medium uppercase tracking-[0.12em] text-storm-water-500 dark:text-sea-mist-300/65"
              >{{ languageStore.t('current.sunrise') }}</span
            >
            <span class="text-[15px] font-semibold text-storm-water-800 dark:text-dune-foam">{{
              todaySunrise
            }}</span>
          ...
          <div class="flex flex-col items-center justify-center gap-1 px-2 py-5 text-center">
            ...
            <svg
              class="size-5 text-orange-500 dark:text-orange-400"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="1.75"
            ...
            <span
              class="text-[10px] font-medium uppercase tracking-[0.12em] text-storm-water-500 dark:text-sea-mist-300/65"
              >{{ languageStore.t('current.sunset') }}</span
            >
            <span class="text-[15px] font-semibold text-storm-water-800 dark:text-dune-foam">{{
              todaySunset
            }}</span>
        ...
        <template v-else-if="precipStore.entries.length > 0">
          ...
          <div class="relative">
              ...
              <div class="relative flex-1">
                ...
                <div class="flex h-full items-end gap-px">
                  <div
                    ...
                    :title="
                      entry.mmPerHour > 0
                        ? languageStore.t('current.barTitle', {
                            time: entry.time,
                            amount: entry.mmPerHour.toFixed(2),
                            intensity: precipitationIntensityLabel(entry.mmPerHour),
                          })
                        : languageStore.t('current.barTitleDry', { time: entry.time })
                    "
                    :aria-label="
                      languageStore.t('current.barAria', {
                        time: entry.time,
                        amount: entry.mmPerHour.toFixed(2),
                      })
                    "
            ...
            <div class="mt-1 flex items-start gap-px">
              ...
              <span
                class="shrink-0 pl-1 text-[9px] leading-tight text-storm-water-400 dark:text-sea-mist-300/35"
                >mm/u</span
              >
            </div>
          </div>
          ...
          <button
            ...
            :class="{
              'border-dutch-orange text-dutch-orange':
                alertStyle === 'rain' || alertStyle === 'soon',
              'border-slate-200 text-storm-water-800 dark:border-slate-800 dark:text-dune-foam':
                alertStyle === 'clear',
            }"
            @click="$emit('open-radar')"
          >
            <!-- Icon -->
            <WeatherIcon :code="rainAlertCode" :intensity="rainAlertIntensity" :size="24" />

            <!-- Message -->
            <span v-if="alertStyle === 'rain'">{{
              languageStore.t('current.fallingNow', { label: rainIntensityLabel })
            }}</span>
            <span v-else-if="alertStyle === 'soon'">
              {{
                languageStore.t('current.rainInMinutes', {
                  label: rainIntensityLabel,
                  minutes: precipStore.minutesUntilRain!,
                })
              }}
            </span>
          ...
          <p
            v-if="precipStore.error"
            class="mt-3 text-center text-xs text-[#7a5422]/90 dark:text-[#e7c48b]/85"
          >
            ⚠️ {{ precipStore.error }}
          </p>
        </template>
          ...
          <div class="flex items-start gap-3 rounded-xl bg-red-500/20 px-4 py-3" role="alert">
            ...
            <div>
              <p class="text-sm font-semibold">
                {{ languageStore.t('current.precipitationUnavailable') }}
              </p>
